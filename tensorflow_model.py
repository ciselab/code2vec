...
class Code2VecModel(Code2VecModelBase):
    ...
    def _log_prediction_scores_during_evaluation(self, results, output_file):
        for original_name, top_predicted_words, scores in results:
            found_match = common.get_first_match_word_from_top_predictions_with_score(
                self.vocabs.target_vocab.special_words, original_name, top_predicted_words, scores)
            if found_match is not None:
                prediction_idx, predicted_word, predicted_score = found_match
                if prediction_idx == 0:
                    output_file.write('Original: ' + original_name + ', predicted 1st: ' + predicted_word + ', score: '
                                      + str(predicted_score) + '\n')
                else:
                    output_file.write('\t\t predicted correctly at rank: ' + str(prediction_idx + 1) + ', score: '
                                      + str(predicted_score) + '\n')
            else:
                output_file.write('No results for predicting: ' + original_name + '\n')
